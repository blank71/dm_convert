...
#tfimport-terraform import google_compute_http_health_check.compute_httphealthcheck  __project__/compute-httphealthcheck
resource "google_compute_http_health_check" "compute_httphealthcheck" {
  provider = google-beta
...
resource "google_compute_target_pool" "compute_targetpool_2" {
  ...
  region = "us-west1"
  health_checks = [
    google_compute_http_health_check.compute_httphealthcheck.id
  ]
  instances = [
