...
from expansion import schema_validation_utils
...
def Validate(properties, schema_name, template_name, imports):
  ...
  if validating_imports:
    ...
    for import_object in schema[IMPORTS]:
      reference_by_name = "name" in import_object
      ...
      if import_name not in imports:
        found_file_by_glob_pattern = False
        if not reference_by_name:
          # for path imports, check if a glob pattern is satisfied
          for filename in imports:
            # we require glob import matches to be case sensitive, which is
            # similar in strictness to non-glob matches
            if fnmatch.fnmatchcase(filename, import_name):
              found_file_by_glob_pattern = True
              break
        if not found_file_by_glob_pattern:
          errors.append(
              ("File '%s' requested in schema '%s' "
               "but not included with imports." % (import_name, schema_name)))
...
class ValidationErrors(Exception):
  ...
  The errors could have occurred either in the schema xor in the properties
